...
var matrix, EigenvalueDecomposition2, Matrix2, SingularValueDecomposition2, matrix_default, inverse2;
var init_matrix = __esm({
  "node_modules/ml-matrix/matrix.mjs"() {
    matrix = __toESM(require_matrix(), 1);
    EigenvalueDecomposition2 = matrix.EigenvalueDecomposition;
  ...
  "src/core/matching/matching.js"() {
    ...
    match = ({ keyframe, querypoints: rawQuerypoints, querywidth, queryheight, debugMode: debugMode2, expectedScale }) => {
      ...
      if (H === null) {
        ...
        if (deformableResult) {
          if (debugMode2) debugExtra.deformableResult = deformableResult;
          return {
            isDeformable: true,
        ...
        case "match":
          ...
          for (let i = 0; i < interestedTargetIndexes.length; i++) {
            ...
            if (result.keyframeIndex !== -1 || result.isDeformable) {
              const screenCoords2 = result.isDeformable ? result.inliers.map((m) => m.querypoint) : result.screenCoords;
              const worldCoords2 = result.isDeformable ? result.inliers.map((m) => m.keypoint) : result.worldCoords;
...
var btnUseDefault = document.getElementById("btn-use-default");
var btnReset = document.getElementById("btn-reset");
var btnStopTracking = document.getElementById("btn-stop-tracking");
var compilerProgressContainer = document.getElementById("compiler-progress-container");
var compilerProgressBar = document.getElementById("compiler-progress-bar");
var compilerStatusText = document.getElementById("compiler-status-text");
var trackerStatusBar = document.getElementById("tracker-status-bar");
var targetDetailsContainer = document.getElementById("target-details-container");
var toggleBio = document.getElementById("toggle-bio");
var toggleMesh = document.getElementById("toggle-mesh");
var togglePoints = document.getElementById("toggle-points");
var valFps = document.getElementById("val-fps");
var valInliers = document.getElementById("val-inliers");
var valMode = document.getElementById("val-mode");
var valOctave = document.getElementById("val-octave");
...
var debugCtx = renderCanvas.getContext("2d");
var controller = null;
var isRunning = false;
var currentTargetImgData = null;
var currentTargetName = "";
var lastFrameTime = performance.now();
var frameCount = 0;
var fps = 0;
var lastScreenCoords = null;
var lastDeformedMesh = null;
var lastFoveaCenter = null;
var activeTracks = /* @__PURE__ */ new Map();
...
async function loadDemoImage() {
  ...
  try {
    ...
    img.onerror = () => {
      logToConsole("Error al cargar la imagen demo. Aseg\xFArate de compilar y colocar el recurso en tests/assets.", "error");
    };
    img.src = "./assets/test-image.png";
  } catch (err) {
    logToConsole(`Error cargando imagen demo: ${err.message}`, "error");
  }
}
function processSelectedImage(img) {
  ...
  logToConsole(`Imagen procesada a resoluci\xF3n de target: ${w}x${h}px`);
...
async function compileAndStartAR() {
  ...
  try {
    ...
    logToConsole("Compilaci\xF3n exitosa. Inicializando AR Controller...");
    ...
    logToConsole("AR Controller configurado. Solicitando acceso a la c\xE1mara...");
    await startCamera();
    ...
    logToConsole("\xA1Visualizaci\xF3n AR en marcha!", "info");
    trackerStatusBar.textContent = "Buscando imagen objetivo...";
  } catch (err) {
    logToConsole(`Error durante compilaci\xF3n/inicio: ${err.message}`, "error");
...
async function startCamera() {
  try {
    ...
    logToConsole(`Fallo al acceder a la webcam: ${err.message}`, "error");
...
function stopCamera() {
  ...
  logToConsole("C\xE1mara apagada.");
}
function renderLoop(time) {
  ...
  try {
    ...
    logToConsole(`Error en renderLoop: ${err.message}`, "error");
    console.error(err);
  }
  requestAnimationFrame(renderLoop);
}
function drawVideoToCanvas(ctx, videoEl, targetW, targetH) {
...
function handleARUpdate(data) {
  ...
  try {
    ...
    if (data.type === "updateMatrix") {
      ...
      const hasDetection = targetIndex !== void 0 && targetIndex >= 0 && worldMatrix !== null;
      if (hasDetection) {
        if (!skipped) {
          ...
          track.octaveIndex = data.octaveIndex !== void 0 ? data.octaveIndex : 0;
          track.skipped = false;
    ...
    logToConsole(`Error en handleARUpdate: ${err.message}`, "error");
    console.error(err);
  }
}
function drawBoundingBox(coords, sx, sy) {
  debugCtx.strokeStyle = "rgba(16, 185, 129, 0.85)";
...
function drawDeformableMesh(deformedMesh, sx, sy) {
  const { vertices, triangles } = deformedMesh;
  debugCtx.strokeStyle = "rgba(236, 72, 153, 0.7)";
...
function drawFoveaAttention(foveaCenter, sx, sy) {
  if (!foveaCenter) return;
  debugCtx.strokeStyle = "rgba(59, 130, 246, 0.5)";
