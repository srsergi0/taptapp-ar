...
function computeLaplacianEigenmaps(points) {
  ...
  const adjacency = Array.from({ length: n }, () => /* @__PURE__ */ new Set());
  for (const [u, v] of edges) {
    adjacency[u].add(v);
    adjacency[v].add(u);
  }
  const L = new Matrix2(n, n);
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const j of adjacency[i]) {
      const d_j = degrees[j];
      if (d_j > 0) {
        L.set(i, j, -1 / Math.sqrt(d_i * d_j));
      }
    }
  }
  const evd = new EigenvalueDecomposition2(L);
...
var OfflineCompiler = class {
  ...
  async _compileMatch(targetImages, progressCallback) {
    ...
    for (let i = 0; i < targetImages.length; i++) {
      ...
      const maxMaps = computeLaplacianEigenmaps(maximaPoints);
      const minMaps = computeLaplacianEigenmaps(minimaPoints);
...
var computeHoughMatches = (options) => {
...
var _mapCorrespondence = ({ querypoint, keypoint, keycenterX, keycenterY, scaleOneOverLogK }) => {
...
var linePointSide = (A, B, C) => {
  return (B[0] - A[0]) * (C[1] - A[1]) - (B[1] - A[1]) * (C[0] - A[0]);
};
var checkFourPointsConsistent = (x1, x2, x3, x4, x1p, x2p, x3p, x4p) => {
...
var checkThreePointsConsistent = (x1, x2, x3, x1p, x2p, x3p) => {
  if (linePointSide(x1, x2, x3) > 0 !== linePointSide(x1p, x2p, x3p) > 0) return false;
  return true;
};
var determinant2 = (A) => {
...
var matrixInverse33 = (A, threshold) => {
...
var multiplyPointHomographyInhomogenous = (x, H) => {
...
var smallestTriangleArea = (x1, x2, x3, x4) => {
...
var quadrilateralConvex = (x1, x2, x3, x4) => {
...
var _areaOfTriangle = (u, v) => {
  const a = u[0] * v[1] - u[1] * v[0];
  return Math.abs(a) * 0.5;
};

// src/core/utils/homography.js
var solveHomography = (srcPoints, dstPoints) => {
...
var _denormalizeHomography = (nH, srcParam, dstParam) => {
...
var CAUCHY_SCALE = 0.01;
var CHUNK_SIZE2 = 10;
var NUM_HYPOTHESES = 100;
var NUM_HYPOTHESES_QUICK = 50;
var computeHomography = (options) => {
...
var _checkHeuristics = ({ H, testPoints, keyframe }) => {
  const mp = [];
...
var _cauchyProjectiveReprojectionCost = ({ H, srcPoint, dstPoint, oneOverScale2 }) => {
...
var _checkHomographyPointsGeometricallyConsistent = ({ H, testPoints }) => {
  const mappedPoints = [];
  for (let i = 0; i < testPoints.length; i++) {
...
var INLIER_THRESHOLD = AR_CONFIG.INLIER_THRESHOLD;
var MIN_NUM_INLIERS = AR_CONFIG.MIN_NUM_INLIERS;
var CLUSTER_MAX_POP = AR_CONFIG.CLUSTER_MAX_POP;
var HAMMING_THRESHOLD = AR_CONFIG.HAMMING_THRESHOLD;
var HDC_RATIO_THRESHOLD = AR_CONFIG.HDC_RATIO_THRESHOLD;
var MAX_MATCH_QUERY_POINTS = AR_CONFIG.MAX_MATCH_QUERY_POINTS;
var match = ({ keyframe, querypoints: rawQuerypoints, querywidth, queryheight, debugMode, expectedScale }) => {
  let debugExtra = {};
...
var _query = ({ node, descriptors, querypoint, queue, keypointIndexes, numPop, isHDC, descSize, isCompact }) => {
...
var _findInlierMatches = (options) => {
...
var estimate = ({ screenCoords, worldCoords, projectionTransform }) => {
  return solvePosePnP({
    screenCoords,
    worldCoords,
    projectionTransform
...
var buildModelViewProjectionTransform = (projectionTransform, modelViewTransform) => {
...
var applyModelViewProjectionTransform = (modelViewProjectionTransform, x, y, _z) => {
...
var computeScreenCoordiate = (modelViewProjectionTransform, x, y, z) => {
  const {
    x: ux,
    y: uy,
    z: uz
...
var TRACKING_THRESH = 5;
var K2_FACTOR = 4;
var ICP_MAX_LOOP = 10;
var ICP_BREAK_LOOP_ERROR_THRESH = 0.1;
var ICP_BREAK_LOOP_ERROR_RATIO_THRESH = 0.99;
var mat = [[], [], []];
var J_U_Xc = [[], []];
var J_Xc_S = [[], [], []];
var refineEstimate = ({
...
var _updateModelViewTransform = ({ modelViewTransform, dS }) => {
...
// tests/deformable-debug-app.ts
var inputTarget = document.getElementById("input-target");
var inputScene = document.getElementById("input-scene");
var dropTarget = document.getElementById("drop-target");
var dropScene = document.getElementById("drop-scene");
var btnDemoTarget = document.getElementById("btn-demo-target");
var btnDemoScene = document.getElementById("btn-demo-scene");
var btnRunDiag = document.getElementById("btn-run-diag");
var targetPreview = document.getElementById("target-preview");
var scenePreview = document.getElementById("scene-preview");
var canvasTarget = document.getElementById("canvas-target");
var canvasScene = document.getElementById("canvas-scene");
var canvasMatches = document.getElementById("canvas-matches");
var canvasWarp = document.getElementById("canvas-warp");
var canvasMesh = document.getElementById("canvas-mesh");
var placeholderTarget = document.getElementById("placeholder-target");
var placeholderScene = document.getElementById("placeholder-scene");
var placeholderMatches = document.getElementById("placeholder-matches");
var placeholderWarp = document.getElementById("placeholder-warp");
var placeholderMesh = document.getElementById("placeholder-mesh");
var badgeTargetPts = document.getElementById("badge-target-pts");
var badgeScenePts = document.getElementById("badge-scene-pts");
var badgeWarp = document.getElementById("badge-warp");
var badgeMesh = document.getElementById("badge-mesh");
var consoleLogs = document.getElementById("console-logs");
var statusDot = document.getElementById("console-status-dot");
var btnFilterAll = document.getElementById("btn-filter-all");
var btnFilterHough = document.getElementById("btn-filter-hough");
var btnFilterInliers = document.getElementById("btn-filter-inliers");
var targetImg = null;
var sceneImg = null;
...
function loadDemoTarget() {
  ...
  img.onload = () => {
    ...
    logToConsole("Target demo cargado correctamente. Iniciando pre-compilaci\xF3n...");
    compileTarget();
  };
  img.onerror = () => {
    logToConsole("Error cargando target demo. Aseg\xFArate de compilar y colocar tests/assets/test-image.png.", "error");
  };
...
function loadTarget(file) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      logToConsole("Target cargado. Iniciando pre-compilaci\xF3n...");
      compileTarget();
    };
...
function loadScene(file) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      logToConsole("Imagen de escena cargada y lista para diagn\xF3stico.", "success");
    };
    img.src = e.target?.result;
  };
  reader.readAsDataURL(file);
}
function updatePreview(container, img, name) {
  container.innerHTML = `
...
async function compileTarget() {
  ...
  logToConsole(`Procesando target a resoluci\xF3n de compilaci\xF3n: ${w}x${h}px`);
  ...
  try {
    ...
    logToConsole(`Compilaci\xF3n de target finalizada con \xE9xito. Tama\xF1o del buffer: ${(compiledBuffer.byteLength / 1024).toFixed(1)} KB`, "success");
  } catch (err) {
    logToConsole(`Fallo al compilar el target: ${err.message}`, "error");
  }
}
...
async function runDiagnostic() {
  ...
  setStatus(false);
  logToConsole("--- INICIANDO DIAGN\xD3STICO DE DETECCI\xD3N Y TRACKING ---", "info");
  try {
    ...
    logToConsole(`Escena escalada a resoluci\xF3n del tracker: ${sceneW}x${sceneH}px`);
    ...
    logToConsole(`DetectorLite: Encontr\xF3 ${sceneFeatures.length} puntos en la escena.`);
    ...
    logToConsole(`Detecci\xF3n Completada:`);
    ...
    if (matchResult.keyframeIndex !== -1) {
      ...
      logToConsole(`- RANSAC Homograf\xEDa: \xA1CONSEGUIDO! ${inliers.length} inliers encontrados.`, "success");
    } else if (isDeformable && debugFrame.deformableResult) {
      hasPose = true;
      logToConsole(`- RANSAC Deformable (Consenso Espectral): \xA1CONSEGUIDO! ${deformableInliers.length} inliers.`, "success");
      const estimator = new Estimator(buildProjectionTransform(sceneW, sceneH));
      modelViewTransform = estimator.estimate({
        screenCoords: deformableInliers.map((m) => m.querypoint),
        worldCoords: deformableInliers.map((m) => ({ x: m.keypoint.x + 0.5, y: m.keypoint.y + 0.5, z: 0 }))
      });
    } else {
      logToConsole(`- Detecci\xF3n fallida: Consensus RANSAC por debajo de los inliers m\xEDnimos (${AR_CONFIG.MIN_NUM_INLIERS}).`, "error");
    }
    let trackerResult = null;
    if (hasPose && modelViewTransform) {
      logToConsole("Simulando fase de Tracking (warp, matching en deformaci\xF3n, y relajaci\xF3n de malla)...");
      ...
      if (trackerResult.deformedMesh) {
        logToConsole(`Malla deformable relajada con \xE9xito: ${trackerResult.deformedMesh.triangles.length / 3} tri\xE1ngulos construidos.`, "success");
      } else {
        logToConsole(`Malla deformable no generada: faltan caracter\xEDsticas suficientes en la regi\xF3n.`, "warning");
      }
    }
    diagnosticData = {
      sceneW,
    ...
    drawMeshView();
    setStatus(true);
    logToConsole("--- DIAGN\xD3STICO FINALIZADO CORRECTAMENTE ---", "success");
  } catch (err) {
    logToConsole(`Error en diagn\xF3stico: ${err.message}`, "error");
    console.error(err);
  }
}
function compilerImport(buffer) {
  const compiler = new OfflineCompiler();
  const result = compiler.importData(buffer);
  const markerDimensions = result.dataList.map((item) => [item.targetImage.width, item.targetImage.height]);
  const trackingDataList = result.dataList.map((item) => item.trackingData);
  const matchingDataList = result.dataList.map((item) => item.matchingData);
  return { markerDimensions, trackingDataList, matchingDataList };
}
function buildProjectionTransform(w, h) {
...
function drawTargetView() {
  ...
  const scaleX = octave0 ? targetW / octave0.w : 1;
  const scaleY = octave0 ? targetH / octave0.h : 1;
...
function drawMatchesView() {
  ...
  canvasMatches.width = targetW + sceneW + 20;
  ...
  const keyframe = diagnosticData.targetMatchingData[0];
  const octave0 = keyframe?.[0];
  const scaleTargetX = octave0 ? targetW / octave0.w : 1;
  const scaleTargetY = octave0 ? targetH / octave0.h : 1;
  ctx.lineWidth = 1.5;
...
function drawWarpView() {
  ...
  if (!hasPose || !trackerResult || !trackerResult.debugExtra?.projectedImage) {
    ...
    const ctx2 = canvasWarp.getContext("2d");
    ctx2.clearRect(0, 0, 100, 100);
    placeholderWarp.style.display = "block";
    placeholderWarp.textContent = "Pose no encontrada. Imposible calcular proyecci\xF3n.";
    return;
  }
  badgeWarp.textContent = "PROYECTADO";
  badgeWarp.className = "diag-badge badge-emerald";
  const projImage = trackerResult.debugExtra.projectedImage;
...
async function autoLoadResources() {
  logToConsole("Iniciando carga autom\xE1tica de recursos por defecto (test-image.png y test-query.jpg)...");
  ...
  try {
    ...
    logToConsole("Recursos cargados correctamente. Compilando e iniciando diagn\xF3stico...");
    await compileTarget();
    runDiagnostic();
  } catch (err) {
    logToConsole(`Error en carga autom\xE1tica: ${err.message}`, "warning");
    logToConsole('Por favor, aseg\xFArate de colocar "test-query.jpg" en "tests/assets/" o c\xE1rgala manualmente usando la barra lateral.', "info");
  }
}
